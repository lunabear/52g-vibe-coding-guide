        ...
        <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
          ...
          <div className="mt-2 grid grid-cols-7 gap-1">
            {Array.from({length:35}).map((_,i)=> (
              <div key={i} className={`h-7 rounded flex items-center justify-center text-[11px] ${i===10?'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]':''}`}>{i%30+1}</div>
            ))}
          </div>
        </div>
        <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4 flex flex-col">
          ...
          <div className="mt-3 flex-1 flex items-end gap-1">
            {Array.from({length:16}).map((_,i)=> (
              <div key={i} className="w-3 rounded-t" style={{height:`${20+Math.random()*60}%`, backgroundColor:'hsl(var(--primary))'}} />
            ))}
          </div>
          <button className="mt-3 px-3 py-2 rounded-md text-[hsl(var(--primary-foreground))] bg-[hsl(var(--primary))] text-xs">설정하기</button>
        </div>
        <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
