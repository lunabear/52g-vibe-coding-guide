...
- 개발자 Bob의 개발 작업 설계: 기술 구현 및 API 연동 가이드
...
- **오류 처리**: 원인 파악용 최소 console.log 삽입 → 근본 원인 해결 → 해결 후 로그 제거
- **에러 로그 관리**: 
  - docs/error-log.md 파일을 생성하여 발생한 에러와 해결 방법을 체계적으로 기록합니다
  - 에러 발생 시: 에러 메시지, 발생 상황, 원인 분석, 해결 과정, 최종 해결책을 상세히 문서화
  - 유사한 에러 재발생 시 기존 로그를 참고하여 빠른 해결을 도모합니다
  - 에러 패턴 분석을 통해 예방 가능한 이슈들을 사전에 식별하고 개선합니다
